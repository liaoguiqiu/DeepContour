...
pub = rospy.Publisher("/OCT_frames_gui", Image, queue_size=10)


def talker():
    pub = rospy.Publisher('chatter', String, queue_size=10)
    rospy.init_node('talker', anonymous=True)
    rate = rospy.Rate(10) # 10hzem
    while not rospy.is_shutdown():
        hello_str = "hello world %s" % rospy.get_time()
        rospy.loginfo(hello_str)
        pub.publish(hello_str)
        rate.sleep()
def img_publisher(current_frame_thr):
    br = CvBridge()
...
class Find_shadow(object):
    def __init__(self):
        ...
        self.port = "5555"
        self.socket.connect("tcp://192.168.12.170:%s" % self.port)
