...
class OCTStab(object):
    def __init__(self,H=256,W=832):

        self.shift_integral = np.zeros(832)

        self.Len_steam = 5
        self.sequence_num  =  0

        self.steam = np.zeros((self.Len_steam, H, W))
        self.steam2 = np.zeros((self.Len_steam, H, W))
        self.save_sequence_num = 0  # processing iteration initial
        self.addition_window_shift = 0  # innitial shifting parameter
        self.Window_ki_error = 0
        self.Window_kp_error = 0
        self.Kp = 0  # initial shifting paramerter
        self.dual_thread = Dual_thread_Overall_shift_NURD()

        # my_netD  = ShiftingNetBody.ShiftingNet_init_my( None)


        pass
    #input image sequence to correct it
    def correct(self, video):
        gray_video = cv2.cvtColor(video, cv2.COLOR_BGR2GRAY)
        H_ori, W_ori = gray_video.shape
        gray_video = cv2.resize(gray_video, (832, H_ori), interpolation=cv2.INTER_LINEAR)
        start_time = time()
        if (self.sequence_num <   5):
            # bffer a resized one to coputer the path and cost matrix
            self.steam = np.append(self.steam, [gray_video ], axis=0)  # save sequence
            # normal beffer process
            self.steam = np.delete(self.steam, 0, axis=0)

            self.steam2 = np.append(self.steam2, [gray_video], axis=0)  # save sequence
            ...
            self.steam = np.append(self.steam, [gray_video ], axis=0)  # save sequence
            # no longer delete the fist  one
            self.steam = np.delete(self.steam, 1, axis=0)
            self.steam2 = np.append(self.steam2, [gray_video ], axis=0)  # save sequence
            self.steam2 = np.delete(self.steam2, 0, axis=0)

            self.dual_thread.input(self.steam, self.steam2, self.Len_steam, self.addition_window_shift)

            self.dual_thread.runInParallel()

            overall_shifting, shift_used1 = self.dual_thread.output_overall()
            shift_mean_error = int(overall_shifting) - int(Overall_shiftting_WinLen / 2)
            ...
            path, Costmatrix, shift_used2 = self.dual_thread.output_NURD()


            self.shift_integral = VIDEO_PEOCESS.fusion_estimation(self.shift_integral, path, addition_window_shift,
                                                             self.Window_ki_error)

            self.Window_ki_error = (self.shift_integral - self.addition_window_shift)   + self.Window_ki_error


            Corrected_img = VIDEO_PEOCESS.de_distortion_integral2(gray_video, self.shift_integral, self.sequence_num)
            ...
            self.steam = np.append(self.steam, [Corrected_img], axis=0)  # save sequence
            # no longer delete the fist  one
            self.steam = np.delete(self.steam, 1, axis=0)
            self.sequence_num +=1
