...
class Auto_json_label(object):
    ...
    def predict_contour(self, gray, H_s, W_s, attatch_rate=0.1, points=64):
        ...
        pix_wise = self.CE_Nets.pix_wise
        pix_wise = pix_wise[0, :, :, :].cpu().detach().numpy() * 255

        if pix_wise.shape[0]>1:
            color = numpy.zeros((pix_wise.shape[1], pix_wise.shape[2], 3))

            color[:, :, 0] =  numpy.clip(pix_wise[0, :, :], 1, 254)
            color[:, :, 1] =  numpy.clip(pix_wise[1, :, :], 1, 254)
            color[:, :, 2] =  numpy.clip(pix_wise[2, :, :], 1, 254)
        else:
            color = numpy.zeros((pix_wise.shape[1], pix_wise.shape[2]))
            color[:,:] =  numpy.clip(pix_wise[0, :, :], 1, 254)
        ...
        cv2.imshow('pix_wise', color.astype(np.uint8))
        cv2.waitKey(1)
