...
pub_orginal_img = rospy.Publisher("/OCT_frames_gui_ori", Image, queue_size=10) # the publisher for hte image
pub_dis = rospy.Publisher("/OCT_dis", numpy_msg(Floats), queue_size=10) # the ublisher for thre ssidtance
pub_fed = rospy.Publisher("/OCT_feed", Float64, queue_size=10) # the ublisher for thre ssidtance
global min_dis_l2
min_dis_l2 = np.zeros(3)
contact_l = np.zeros(3)
last_dy =0
def talker():
    ...
    while not rospy.is_shutdown():
        ...
        rate.sleep()
def img_publisher(current_frame_ori,current_frame_thr,y1,y2):
    global min_dis_l2
    global contact_l
    global last_dy
    br = CvBridge()
    #pub.publish(br.cv2_to_imgmsg(current_frame_thr))
    pub.publish(br.cv2_to_imgmsg(current_frame_thr))
    pub_orginal_img.publish(br.cv2_to_imgmsg(current_frame_ori))
    ...
    print("distance" + str(min_dis))
    # pack the distance, contact and the orientation together
    orientation =360* min_idex/len(y) # use the 360 degree to uniform the orientation
    msg   = np.array([min_dis,contact,orientation] ) .astype("float32")
    dy = -(min_dis - 0 - contact)  # this is horizontal
    #dy = 0.7*last_dy + 0.3*dy
    last_dy = dy
    #msg2 = np.array([dy] ) .astype("float32")
    msg2 = Float64()
    msg2.data = dy
    pub_dis.publish(msg)
    pub_fed.publish(msg2 )
