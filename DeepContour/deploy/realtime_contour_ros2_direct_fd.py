...
def img_publisher(current_frame_ori,current_frame_thr,y1,y2):
    ...
    # TODO:new publisher, no publish
    target_y = 20
    current_oct_dis =min_dis
    current_oct_contact = contact
    current_oct_orientation = orientation
    dy = -(current_oct_dis - target_y - current_oct_contact)  # this is horizontal
    #msg   = np.array([min_dis,contact,orientation] ) .astype("float32")
    msg   = np.array([dy] ) .astype("float32")
